...
def test_getting_favourite_tv_show
  result = get_favourite_tv(@person4)
  ...
  def test_likes_to_eat_
    result = likes_to_eat(@person3, "tuna")
    assert_equal(false, result)
  end

  def test_likes_to_eat_positive
    result = likes_to_eat(@person3, "Scooby snacks")
    assert_equal(true, result)
  end

  # 4. For a given person, add a new name to their list of friends
  # (e.g. the function add_friend(@person2, "Scrappy-Doo") should add
  # Scrappy-Doo to the friends.)
  ...
  # to the other
  # (hint: our function will probably need 3 arguments passed to it... the lender,
  #the lendee, and the amount for this function)
  # (hint2: You should test if both the lender's and the lendee's money have
  #changed, maybe two assertions?)

  def test_lending_lender
    result = money_lending(@person5, @person4, 20)
    assert_equal(80, result)
  end

  def test_lending_lendee
    result - money_lending(@person5, @person4, 20)
